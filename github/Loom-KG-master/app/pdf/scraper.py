...
def scrape_pdf(files_list):
    global filename
    try:
        # Execution context needs to renewed for each file, don't move the for loop down
        for files in files_list:
            ...
            credentials = (
                ...
                .from_file(".\Validation\pdfservices-api-credentials.json")
                .build()
            )
            ...
            source = FileRef.create_from_stream(files.read(), "application/pdf")
            ...
            result.save_as("./outputZip/" + files.filename + ".zip")
        return True
